...
fdi <- read_csv("~/Code/CompText_Jour/data/ChinaFDI-LAT_tidy.csv")
...
stories <- str_replace_all(fdi$text, "- ", "")
...
```{r}
stories_bigrams_cts <- fdi %>% 
  select(text) %>% 
  mutate(text = str_squish(text)) |> 
  mutate(text = tolower(text)) |>  
  mutate(text = str_replace_all(text, "calif|database central|info| times| issn|publication|periodicals|database|central|url|http|caption|photo|issn|proquest|document|copyright|accountid|docview|search.proquest.com|los angeles|los angeles times|los angeles, calif\\.", "")) |> 
