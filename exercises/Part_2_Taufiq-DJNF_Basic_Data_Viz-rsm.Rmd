...
```{r}
# If the correct column is `day`, use that to count the number of calls per day
day_calls <- SF %>% 
  count(call_date) %>% 
  arrange(desc(n))

# View the day with the most calls
day_calls
#rsw comment - write out the answers
#The highest day with calls was 2019-08-15, with 232 calls in one day
```

**2) Chart Calls by Day** Use the code above and add a ggplot chart to it

```{r}
# Create a bar chart for the number of calls by day
ggplot(day_calls, aes(x = call_date, y = n, fill = n)) +
  geom_col(position = "dodge") + 
  theme(legend.position = "none") +
  labs(title = "311 Calls for Service By Day, San Francisco",
       subtitle = "SF PD Service Call Data, 2016-2019",
       ...
       y="Number of Calls",
       x="Date")
#rsw comment - not the correct graphic
#RSW comment - charting it on an actual date field yields a better graphic and one that makes sense
#You needed to build the table of day calls by counting on an actual date field, call_date2
```
...
```{r}
#rsw comment- this code is in the wrong place. it should be on line 280 to answer the max calls question.
# Identify the top 5 days with the most calls
top_5_days <- day_calls %>% 
  slice_max(n, n = 5)
...
#rsw comment - it appears you erased this question
**Question**: Identify the top 5 days with the fewest calls?
