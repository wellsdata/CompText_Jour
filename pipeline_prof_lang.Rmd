...
one_word_per_row <- dataframe %>% 
  select(text, linenumber, headline) %>% 
...
afinn_by_linenumber <- one_word_per_row %>% 
...
afinn_by_headline <- one_word_per_row %>% 
  inner_join(get_sentiments("afinn")) %>% 
  group_by(index = headline) %>% 
  summarise(sentiment = sum(value)) %>% 
  mutate(method = "AFINN")
